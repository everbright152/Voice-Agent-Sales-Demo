      ...
      <div className='min-h-screen bg-[#080d1a]'>
        <div className='max-w-4xl mx-auto px-4 pt-8 mb-8'>
          <div className='flex items-center justify-center gap-2'>
            {[{key:'niche',label:'1. Pick Niche'},{key:'crawl',label:'2. Enter URL'},{key:'edit',label:'3. Review'},{key:'demo',label:'4. Demo'}].map((s, i) => (
              <div key={s.key} className='flex items-center'>
                <div className={'px-3 py-2 rounded-full text-xs font-medium ' + (step === s.key ? 'bg-green-500 text-white' : 'bg-slate-800 text-slate-500 border border-white/10')}>
                  {s.label}
                </div>
        ...
        <div className='max-w-6xl mx-auto px-4 pb-20 space-y-8'>
          {step === 'niche' && <NicheSelector onSelect={(n) => { setSelectedNiche(n); setStep('crawl'); }} />}
          {step === 'crawl' && selectedNiche && (
            <WebsiteCrawler niche={selectedNiche} onDataExtracted={(d) => { setBusinessData(d); setStep('edit'); }} onBack={() => setStep('niche')} />
          )}
          {step === 'edit' && businessData && selectedNiche && (
            <BusinessDataEditor data={businessData} niche={selectedNiche} onConfirm={(d) => { setBusinessData(d); setStep('demo'); }} onBack={() => setStep('crawl')} />
          )}
          {step === 'demo' && businessData && selectedNiche && (
            <div className='grid grid-cols-1 lg:grid-cols-3 gap-6'>
              <div className='lg:col-span-2 space-y-6'>
                <DemoCallPanel businessData={businessData} niche={selectedNiche} onCallStarted={() => { setIsCallActive(true); setWorkflowStep(1); }} onCallEnded={() => { setIsCallActive(false); setWorkflowStep(0); }} isCallActive={isCallActive} />
          ...
          {step === 'demo' && (
            <div className='text-center'>
              <button onClick={() => { setStep('landing'); setSelectedNiche(null); setBusinessData(null); }} className='text-slate-400 hover:text-white text-sm underline'>
                Back to home
              </button>
            </div>
          )}
      ...
      <section className='relative flex flex-col items-center justify-center min-h-screen px-4 py-24 text-center overflow-hidden'>
        ...
        <div className='relative z-10 max-w-5xl mx-auto'>
          <div className='inline-flex items-center gap-2 bg-green-500/10 border border-green-500/20 rounded-full px-5 py-2 text-green-400 text-sm font-medium mb-8'>
            ...
            AI Voice Agent &bull; Live Demo &bull; Powered by Retell AI
          </div>
          ...
          <p className='text-xl text-slate-400 max-w-2xl mx-auto mb-12 leading-relaxed'>
            Enter your website URL. We build a custom AI voice agent for your business in 60 seconds.
            <strong className='text-white'> Watch it answer calls, capture leads, and book appointments.</strong>
          </p>
          <div className='flex flex-col sm:flex-row gap-4 justify-center mb-16'>
            <button onClick={() => setStep('niche')} className='group bg-gradient-to-r from-green-500 to-emerald-500 hover:from-green-400 hover:to-emerald-400 text-white font-bold text-lg px-10 py-5 rounded-2xl shadow-2xl shadow-green-500/25 flex items-center justify-center gap-3'>
              <span>Build My AI Agent</span>
              <span>&#8594;</span>
            </button>
            <a href='tel:+12764479695' className='flex items-center justify-center gap-3 bg-white/5 hover:bg-white/10 border border-white/20 text-white font-bold text-lg px-10 py-5 rounded-2xl'>
      ...
      <section className='py-20 px-4'>
        <div className='max-w-5xl mx-auto'>
          ...
          <div className='grid grid-cols-1 sm:grid-cols-4 gap-4'>
            {[{step:'01',icon:'&#127919;',title:'Pick Your Niche',desc:'Legal, beauty, healthcare, home services, or financial'},{step:'02',icon:'&#127760;',title:'Enter Your URL',desc:'We scan your website and pull in your info automatically'},{step:'03',icon:'&#129302;',title:'AI Agent Built',desc:'A custom voice agent configured with your exact services and hours'},{step:'04',icon:'&#128222;',title:'Watch It Work',desc:'Start a live demo call and hear it answer as YOUR business'}].map((item) => (
      ...
      <section className='py-24 px-4'>
        <div className='max-w-3xl mx-auto'>
          <div className='relative overflow-hidden rounded-3xl border border-green-500/30 bg-gradient-to-br from-green-900/30 via-slate-900 to-blue-900/20 p-12 text-center'>
            ...
            <p className='text-slate-400 text-lg mb-8'>Enter your website URL. Free demo. 60 seconds.</p>
            <button onClick={() => setStep('niche')} className='bg-gradient-to-r from-green-500 to-emerald-500 hover:from-green-400 hover:to-emerald-400 text-white font-black text-xl px-12 py-5 rounded-2xl shadow-2xl shadow-green-500/25'>
              Build My Free Demo &#8594;
            </button>
